...
document.addEventListener("DOMContentLoaded", () => {
	...
	createAccountForm.addEventListener("submit", e => {
        ...
        setFormMessage(createAccountForm, "error", "One of the entries is invalid");
    });
    ...
    document.querySelectorAll(".form__input").forEach(inputElement => {
		...
		inputElement.addEventListener("blur", e => {
            if (e.target.id === "signupEmail" && e.target.value.length > 0 && e.target.value.length < 3) {
                setInputError(inputElement, "Must eneter a vaild email");
            }
        });
		...
		inputElement.addEventListener("blur", e => {
            if (e.target.id === "signupConfirmPassword" && e.target.value.length > 0 && e.target.value.length < 8) {
                setInputError(inputElement, "Password must be match");
            }
        });
